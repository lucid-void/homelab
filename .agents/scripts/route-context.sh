...
# media-stack.md explicitly excludes plex/, romm/, minecraft*/ (they keep
# their own decision files) — checked separately from the big case below
# because extglob's !(...) negation isn't segment-bounded: it would also
# swallow "plex/app/foo.yml" as "not literally plex" and wrongly fire.
case "$P" in
  kubernetes/apps/media/plex/*|kubernetes/apps/media/romm/*|kubernetes/apps/media/minecraft*/*) ;;
