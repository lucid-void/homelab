...
| manga stack | Tranga (downloader) + Kavita (reader) in `media` namespace, both `app-template` v3.7.3, writing/reading `media-nfs` subPath `Manga`. **Chosen because Kaizoku + its mangal engine are both archived/unmaintained** — do not deploy those. Tranga is a split FE/BE in one HelmRelease: controller `website` (`glax/tranga-website`, nginx, port 80) reverse-proxies `/api/` to controller `api` (`glax/tranga-api`, port 6531) via `API_URL=http://tranga-api:6531`; only `tranga-website` gets an HTTPRoute. tranga-api runs as UID:GID (pod `securityContext` 2202/2200, no PUID env), connects to CNPG via **separate** `POSTGRES_HOST` (incl. `:5432`)/`POSTGRES_DB`/`POSTGRES_USER`/`POSTGRES_PASSWORD` env — password via `secretKeyRef` on reflected `tranga-role-secret`, **no db-bootstrap Job** (unlike Gitea). Tranga has no auth of its own. **Tranga publishes only rolling channel tags (`latest`, `oldstable`, `cuttingedge`) — no semver**; `latest` used as an accepted deviation from the image-pin policy. **FlareSolverr** (`ghcr.io/flaresolverr/flaresolverr` v3.5.0) runs as a 3rd Tranga controller (service `tranga-flaresolverr:8191`, Chrome-based, runs as root in its own pod); `FLARESOLVERR_URL` on tranga-api lets Cloudflare-gated connectors (WeebCentral, Bato) work — without it they 403 and chapter lists come back empty (Tranga auto-appends `/v1`). Kavita's first admin is created by an idempotent `kavita-bootstrap` Job (POST `/api/account/register`; first user auto-becomes admin; creds from `kavita-admin-secret` SealedSecret; HTTP 400 = admin already exists, treated as success). **Tranga gotcha:** `FLARESOLVERR_URL` env only seeds a *fresh* `settings.json` (on the config PVC); an existing `settings.json` with an empty value wins on load, so after adding FlareSolverr to an already-initialized instance you must set it at runtime — `PATCH /v2/Settings/FlareSolverr/Url` writes the file but the live value only updates after a pod restart (the `[FromBody] string` is finicky; editing `settings.json` directly + restart is the reliable path). Verify with `POST /v2/Settings/FlareSolverr/Test` (200 = working). **Connector note:** licensed series (e.g. Witch Hat Atelier) have ~no chapters on MangaDex — add them from WeebCentral (Cloudflare-gated, needs FlareSolverr). |
