...
**A ConfigMap mounted under `/opt/couchdb` kills CouchDB silently.** The image's entrypoint runs `find /opt/couchdb ! -user couchdb -exec chown -f couchdb:couchdb '{}' +` under `set -e` (`docker-entrypoint.sh:14,43`). `chown -f` suppresses the error *message* but still returns **1** on a read-only mount, `find -exec +` propagates it, and the script aborts — giving **exit 1 with completely empty logs** and `startedAt` == `finishedAt`. Hit on first deploy with the config mounted at `local.d/…` via `subPath`. Dropping `readOnly: true` does not help (ConfigMap volumes are always read-only in k8s); the file must be **copied by an initContainer onto an emptyDir** that becomes `/opt/couchdb/etc/default.d`. Generalises: any read-only mount under a directory some entrypoint recursively chowns is a silent-exit trap, and empty logs plus a zero-second lifetime is the signature.

**CouchDB `NODENAME` is load-bearing and silent when wrong.** CouchDB keys its shard paths by erlang node name. The official image's `vm.args` ships with **no `-name` line**, so `docker-entrypoint.sh` appends `-name couchdb@$NODENAME`; with `NODENAME` unset the VM derives a name from the container hostname, which on a Deployment changes every rollout. The pod then comes back healthy under a *different* node identity and reports an **empty database** while the real shards sit untouched on disk. Pinned to `127.0.0.1` — this node never clusters, a long erlang node name must resolve at VM boot, and the literal keeps DNS off the startup path. **Never change it once data exists**; a restore must go back to the same value or the restored shards are unreadable.

**CouchDB health probes must be authenticated `exec`, not `httpGet`.** `[chttpd] require_valid_user = true` gates **`/_up` as well**, so a plain `httpGet: /_up` returns 401 and the kubelet restart-loops a perfectly healthy pod — same shape as the Joplin `Host`-header trap below. The probes shell out to `curl -u "$COUCHDB_USER:$COUCHDB_PASSWORD"`, reading credentials from the env so nothing lands in the manifest. Note upstream's own `apache/couchdb-helm` makes exactly this switch under the same setting rather than using CouchDB's `require_valid_user_except_for_up` exemption.
...
**Joplin backup needs podAffinity, unlike immich-backup.** `joplin-blobs` is RWO and the job must mount it *while the Joplin pod still holds it* (scale-down happens inside the script), so the job is pinned to the app's node. `immich-backup` skips this only because `immich-library` is RWX. **`obsidian-backup` must NOT have it**, for a different reason again: `couchdb-data` is `openebs-hostpath`, so its PV already carries nodeAffinity from first bind and the scheduler is constrained anyway — while podAffinity on the app pod would make the job unschedulable whenever couchdb is already at 0 replicas, exactly the state a prior failed run leaves behind. The rule is RWO **plus `nfs-client`**, not RWO alone.
