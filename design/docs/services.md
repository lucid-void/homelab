...
| CrossWatch | `crosswatch.blackcats.cc` | `ghcr.io/cenodude/crosswatch:v0.12.3` | `crosswatch-config` PVC (`nfs-client`) — Plex↔Jellyfin↔Trakt watch-history sync; see `design/decisions/watch-sync.md` |
...
| CrossWatch | `https://crosswatch.blackcats.cc/auth/oidc/callback` | Keycloak OIDC, gated on the `user` client role. Path owned by `api/authOidcAPI.py` — confirm it against the app's own OIDC settings screen before the first login attempt |
