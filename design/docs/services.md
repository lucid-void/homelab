...
| mc-router | — (LoadBalancer `172.16.20.52:25565`) | `itzg/mc-router:1.45.2` | none |
...
*Routing:* Minecraft is raw TCP and never touches the Gateway. `mc-router` holds `172.16.20.52:25565` (its own pool-b address, pinned with `lbipam.cilium.io/ips` — it must **not** share an IP with Plex; see `networking.md`) and demultiplexes on the hostname in the client handshake. It discovers backends by watching Services for `mc-router.itzg.me/externalServerName`, so adding a server is a new HelmRelease plus that annotation plus a name on the mc-router Service's `external-dns` hostname list. This is the only reason external-dns has `service` in its `sources`.
